...
def prepare_data(share_events_df, event_properties_df):
...
def clean_data(share_events_df):
    ...
    # preparatory cleaning of the data before it can be graphed
    prepare_data(share_events_df, event_properties_df)

    return users_df, share_events_df, event_properties_df


def read_mongo_data(collection_name, date_limit):
    ...
    final_data = clean_data(records)
    sql_insert(final_data)
